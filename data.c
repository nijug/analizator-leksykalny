...
int counter_exist(*char word) 
{
	for (int i = 0; i < counter_size; i++)
	{
		if (strcmp(counter[i]->names, word) return i;
	}
...
void store_add_call(*char fun_name, int line, *char file)
{
	...
	{
		create(fun_name);
		all_data[data_size - 1]->call_f[call_iterator] = file;
		all_data[data_size - 1]->call_l[call_iterator] = line;
		call_iterator++;
	}
	else
	{
		all_data[i]->call_f[call_iterator] = file;
		all_data[i]->call_l[call_iterator] = line;
		call_iterator++;
	}
	history[history_size]->names = fun_name;
...
void write_data() // dane przechowywane globalnie, wystarczy wywolac funkcje na koniec main i zamknac program
{
	for (int i = 0; i < data_size; i++)
	{
		...
		else
		{
			...
			{
				pair counter = malloc(100 * sizeof * pair);
				int counter_size = 0;
				for (int j = end; j <= start; j++)
				{
					int temp = counter_exist(history[j]->names);
					if (temp == -1)
					{
